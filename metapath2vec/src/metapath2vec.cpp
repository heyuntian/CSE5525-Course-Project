...
int binary = 0, cbow = 0, debug_mode = 2, window = 5, min_count = 5, num_threads = 40, min_reduce = 1;
int pp = 1;
int *vocab_hash;
...
const int m_base = 0, g_base = 45843, c_base = 45863, u_base = 162321;
const int m_num = 45843, g_num = 20, c_num = 116458, u_num = 270896;
...
std::vector<long long> m_table(0), g_table(0), c_table(0), u_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    else if (id >= c_base) { // subforum
      c_table.push_back(target);
    }
    else if (id >= g_base) { // post
      g_table.push_back(target);
    }
    else if (id >= m_base) {
  ...
  std::cout << "g_table.size() " << g_table.size() << std::endl;
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          if (d == 0) {
            ...
            if (pp) {
              if (last_word >= u_base) { // user
                target = u_table[(next_random >> 16) % u_table.size()];
              }
              else if (last_word >= c_base) { // cast
                target = c_table[(next_random >> 16) % c_table.size()];
              }
              else if (last_word >= g_base) { // genre
                target = g_table[(next_random >> 16) % g_table.size()];
              }
              else if (last_word >= m_base) { // genre
