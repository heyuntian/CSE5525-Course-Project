...
template<typename T>
void readAttrs(ifstream *f, vector<vector<T>> *v, vector<vector<T>> *inv_v, int base1, int base2, int n) {
	if (!(*f).is_open())
        exit(EXIT_FAILURE);
    int mId, num_attr, i, j, attr;
    while (!(*f).eof()) {
    	(*f) >> mId >> num_attr;
    	ASSERT(mId - base1 < n);
    	ASSERT(mId - base1 >= 0);
    	...
    	if (num_attr > 0) {
    		for (j = 0; j < num_attr; j++) {
    			...
    			ASSERT(attr - base2 >= 0);
    			attr -= base2;
    			(*v)[mId].push_back(attr);
    			(*inv_v)[attr].push_back(mId);
    		}
    	}
    }
}
